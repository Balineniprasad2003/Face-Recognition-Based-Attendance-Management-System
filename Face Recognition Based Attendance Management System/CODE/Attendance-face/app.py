    ...
    while True:
        ...
        for x, y, w, h in faces:
            ...
            if conf <= detection_threshold:
                ...
                s_name = student['name'].iloc[0]  # Use .iloc to safely access the first row
                s_email = student['email'].iloc[0]
                ...
                if current_frame_count >= max_frame_detection_count:
                    ...
                    is_cam = False
                    db , cursor = get_db()
                    current_time = (datetime.now() + timedelta(hours=1,minutes=0)).time()
...
def marksback():
    ...
    if request.method == 'POST':
        ...
        cursor.execute(f"select id from mid_results where mcomp = '{m_comp}' and rno = '{rno}'")
